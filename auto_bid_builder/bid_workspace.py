...
def create_workspace(base_root: str | Path, opportunity_row: dict) -> Path:
    """Create a guided local bid workspace from a normalized opportunity row."""
...
def _review_markdown(root: Path, payload: dict) -> str:
    ...
    lines = [
        ...
        f"Millwork-relevant pages found: {payload.get('relevant_page_count', 0)}",
        "",
    ]
    ...
    lines.extend(["## Highest-priority pages", ""])
    for row in payload.get("pages", [])[:40]:
        sheet = f" / sheet {row.get('sheet')}" if row.get("sheet") else ""
        terms = ", ".join(row.get("scope_terms", [])) or "general millwork signal"
        lines.append(
            f"- **{row.get('source')} - page {row.get('page')}{sheet}** - score {row.get('relevance_score')} - {terms}"
        )
    ...
    if not payload.get("pages"):
        lines.append("- No pages met the current millwork signal rules. The files may be image-only scans or may need manual review.")
    lines.extend(
        [
            "",
            "## Next estimator action",
            "",
            "Review the highest-priority pages against the responsibility schedule, fixture schedule, finish schedule, elevations/details, specifications, and any addenda before accepting scope or pricing assumptions.",
            "",
        ]
    )
    return "\n".join(lines)
...
def analyze_workspace(root: str | Path) -> dict:
    ...
    analysis.update(
        {
            "status": "complete" if pdfs and not errors else ("partial" if pdfs else "no_pdfs"),
    ...
    workflow["documents_added"] = bool(manifest.get("documents")) or bool(pdfs)
    workflow["documents_reviewed"] = bool(pdfs)
    save_manifest(root, manifest)
    return payload
